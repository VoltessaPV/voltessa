...
| Settlement Energy (per interval) | Real exported/imported kWh for one 15-minute interval | Never stored | Derived — meter counter delta (Prosumer) or inverter-power integration (Producer) | `active_cap`/`reverse_active_cap` counters, or integrated `active_power` | `lib/telemetry/energy-metrics.ts`: `getPlantSettlementEnergySeries` / `getPlantProductionEnergySeries` | Market (Revenue, chart bars); Dashboard (Exported/Imported Today); Digital Twin (scenario transform target, then unchanged Settlement Engine); Admin Reporting (`/admin/reporting` — Grid Export / Grid Consumption / PV Production columns); PV Impact Simulator (`/admin/pv-simulator` — `getPlantProductionEnergySeries` is the reference-plant production, linearly capacity-scaled per ADR-023; never re-derived) |
...
| Self-Consumption | Energy produced and used on-site, same interval/day | Never stored | Derived — exactly one identity, `production − export` | — | `lib/telemetry/energy-metrics.ts`: `computeConsumedFromPv` | Dashboard (Consumed from PV); Digital Twin Prosumer recompute; Admin Reporting (PV Consumption column; Total Consumption column uses the `production + import − export` energy-flow identity) |
| Revenue | Σ over settlement intervals of (exported kWh × that interval's real price) | Never stored | Derived, pure function over (price series, settlement series); the per-interval term is `computeIntervalExportRevenueEur` (`exported kWh × price ÷ 1000`), which `computeExportRevenue` also calls | — | `lib/market-price/revenue.ts`: `computeExportRevenue` / `computeIntervalExportRevenueEur` | Dashboard, Market (Revenue card); Digital Twin (Current vs. Simulated comparison); Admin Reporting (Revenue column + `TOTAL` Revenue) |
| Average Selling Price | Revenue ÷ exported energy (export-energy-weighted) | Never stored | Derived, same function as Revenue | — | `lib/market-price/revenue.ts`: `computeExportRevenue` | Market (Revenue row, ASP chart for Week/Month/Year); Admin Reporting (`TOTAL` Price row — weighted average, never a sum) |
