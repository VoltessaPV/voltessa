...
import { selectBestShortTierRowPerInterval, MIN_QUALIFYING_SLOTS_PER_DAY } from "@/lib/forecast/ml/genuine-vintage";

/**
 * Multi-Horizon Self-Learning Forecast milestone, extracted (Continuous
 * Retraining Loop milestone) out of `scripts/ml/export-training-dataset.ts`
 * so the exact same dataset-building logic has TWO callers — that script
 * (writes the result to `ml-forecasting/data/training-dataset.json` for a
 * local/manual `train.py` run) and the new
 * `app/api/internal/forecast/ml-retrain-export/route.ts` (returns the same
 * result over HTTP, for the production retraining timer — see that
 * route's own doc comment for why: `train.py` needs a real Python runtime
 * and CPU time this app's own Vercel deployment cannot provide, so it runs
 * on the Scaleway VM instead, which cannot safely host this app's full
 * pnpm/Prisma dependency tree on its own limited disk — exactly the same
 * reasoning that already put the ONNX Inference Service on that VM
 * instead of Vercel).
 *
 * This file is a pure move, not a rewrite — every line of actual dataset-
 * construction logic (walk-forward TRUE_VINTAGE/RETROSPECTIVE_REPLAY
 * selection, the three simulated lead-time scenarios, leakage-safe
 * feature building) is unchanged from the original script. See that
 * script's own remaining top doc comment for the full design rationale.
 */
...
export type TrainingDataset = {
  ...
  intervalFeatureNames: readonly string[];
  dailyFeatureNames: readonly string[];
  exportedAt: string;
  plantCount: number;
  dayCount: number;
  trueVintageDays: number;
  replayDays: number;
  tierRowCounts: Record<ForecastHorizonTier, number>;
 ...
 * (>10d) boundaries. See this file's own top doc comment and the original
 * script's remaining doc comment for the full SHORT/MEDIUM/LONG and
 * TRUE_VINTAGE/RETROSPECTIVE_REPLAY design rationale — unchanged here.
 */
export async function buildTrainingDataset(): Promise<TrainingDataset> {
  ...
  for (const plant of plants) {
    ...
    for (const date of days) {
      ...
      // Continuous Retraining Loop milestone (fix): TRUE_VINTAGE eligibility is per-INTERVAL,
      // not a raw per-day row count — see genuine-vintage.ts's own top doc comment for why the
      // old exact-96-rows check could never match again once multiple vintages started
      // overlapping the same calendar day (the permanent, steady-state condition).
      const shortTierReconciledRows = await prisma.pvForecastRecord.findMany({
        where: { plantId: plant.id, targetIntervalStart: { gte: dayStart, lt: dayEnd }, actualKwh: { not: null }, horizonTier: "SHORT" },
        orderBy: { targetIntervalStart: "asc" },
      });
      const trueVintageRows = selectBestShortTierRowPerInterval(shortTierReconciledRows);
      const isTrueVintage = trueVintageRows.length >= MIN_QUALIFYING_SLOTS_PER_DAY;
