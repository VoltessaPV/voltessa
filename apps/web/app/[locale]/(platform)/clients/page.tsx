...
/**
 * Trader Workspace milestone. The trader's client portfolio - built for
 * browsing and getting into a client quickly, not for managing records
 * (editing/removing an assignment stays Platform Admin's job under
 * `/admin/assignments`). Selecting a client here is a context switch, not
 * a page navigation - it redirects back to `redirectTo` (whatever page the
 * trader arrived from via the header's "Switch client" link, or this page
 * itself by default), never unconditionally to Dashboard.
 */
export default async function ClientsPage({ searchParams }: Props) {
  const access = await requireTraderOrganizationAccess();
  const params = await searchParams;

  const clients = await listTraderClients(access.trader.id, params.q);
